...
Gracefully stop and remove this exact sandbox. Unlike [`remove_persisted()`](#sb-remove_persisted), `destroy` accepts a running sandbox and converges it through shutdown before deleting persisted state.
...
Use the same state convergence as [`connect_or_start()`](#h-connect_or_start), but start a local stopped sandbox in detached/background mode. Connecting to an already-running sandbox never changes its ownership.
