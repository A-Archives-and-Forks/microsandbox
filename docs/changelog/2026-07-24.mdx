...
Snapshots created on v0.6.6 and v0.6.7 are now fully interchangeable. Microsandbox automatically migrates older `manifest.json` artifacts to the finalized `snapshot.json` descriptor on startup, explicit open, reindex, and archive load, enforcing sparse payload integrity and rejecting duplicate keys. It retains the original descriptor bytes so a downgrade produces the exact prior artifact.
