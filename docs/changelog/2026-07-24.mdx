...
Snapshots created on v0.6.6 and v0.6.7 are now fully interchangeable. On startup, explicit open, reindex, and archive load, microsandbox automatically migrates older `manifest.json` artifacts to the finalized `snapshot.json` descriptor, with mandatory sparse payload integrity and duplicate-key rejection. The original descriptor bytes are retained so a downgrade produces the exact prior artifact.
